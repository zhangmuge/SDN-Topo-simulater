...
class MovingTargetDefense(app_manager.RyuApp):
    ...
    # parser = optparse.OptionParser('usage:\n python3 scapy_arp_scan.py --network network --num num')
    ...
    network="172.18.1.0"
    num=16
    # run("0.0.0.0",port=6789)
    ip_line = network.split(".")
    l_R=[]
    l_Vo=[]
    V2R_Mappings={}
    for ip_last in range(1,num+1):
        ip_line[3]=str(ip_last)
        ip_R='.'.join([str(s) for s in ip_line])
        l_R.append(ip_R)
        l_Vo.append("")
    R2V_Mappings=dict(zip(l_R,l_Vo))
    Resources=[]
    for ip_last in range(num+2,num*4):
        ip_line[3]=str(ip_last)
        ip_R='.'.join([str(s) for s in ip_line])
        Resources.append(ip_R)
    print(R2V_Mappings)
    print(Resources)
    ...
    def update_resources(self,ev):
        ...
        f=open('mtd.log'.format(t),'w')
        print (self.R2V_Mappings,file=f)
        f.close()
